<?php
require_once 'header.php';

?>

<!-- <span>
  <H3><strong>NOS PRODUITS</strong></H3>
</span> -->
<!-- shop section -->

  <section class="shop_section layout_padding" id="quartz">
    <div class="container">
      <div class="heading_container heading_center">
        <H3>
         MONTRES <b>à quartz</b>
        </H3>
      </div>
      <div class="row">
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1674844819033.png">
              <div class="img-box">
                <img src="images\1674844789640.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                 Montre avec jour et date
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1674844977107.png">
              <div class="img-box">
                <img src="images\1674844977107.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre à quartz avec date
                </h6>
               <!--  <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1674844876148.png">
              <div class="img-box">
                <img src="images\1695466489698.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre à quartz avec date
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1674852221957.png">
              <div class="img-box">
                <img src="images\1674852212106.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre à quartz avec date
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images/1681039133186.png">
              <div class="img-box">
                <img src="images/1681038954016.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                   Montre à quartz
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1681039067332.png">
              <div class="img-box">
                <img src="images/1681039061375.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre à quartz
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466489574.jpg">
              <div class="img-box">
                <img src="images/1695466489756.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                 Montre à quartz avec date
                </h6>
               <!--  <h6>
                  prix
                  <span>
                    $400
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1681040163718.png">
              <div class="img-box">
                <img src="images/1681040160417.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre à quartz (dame)
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>
  </section>
  <!--SUITE-->
    <section class="shop_section layout_padding" id="squartz">
    <div class="container">
      <div class="heading_container heading_center">
      </div>
      <div class="row">
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\WhatsApp Image 2023-01-28 at 00.15.20.jpeg">
              <div class="img-box">
                <img src="images\WhatsApp_Image_2023-01-28_at_00.15.30-removebg-preview.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre à quartz avec date
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1681040579572.png">
              <div class="img-box">
                <img src="images\1681040576493.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre à quartz (dame)
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1674242216180.png">
              <div class="img-box">
                <img src="images\WhatsApp_Image_2023-01-20_at_11.43.19-removebg-preview.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre avec date et chrono
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\WhatsApp Image 2023-02-26 at 02.55.45.jpeg">
              <div class="img-box">
                <img src="images\WhatsApp_Image_2023-01-28_at_00.14.54-removebg-preview.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre avec date et chrono
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\WhatsApp Image 2023-02-26 at 07.07.19.jpeg">
              <div class="img-box">
                <img src="images/WhatsApp Image 2023-02-26 at 07.07.41.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre avec date et chrono
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1681039987191.png">
              <div class="img-box">
                <img src="images/1681039983575.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre à cadran rotatif
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1681040235782.png">
              <div class="img-box">
                <img src="images/1681040233250.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Cadran rotatif numéroté
                </h6>
               <!--  <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images/1674242444207.png">
              <div class="img-box">
                <img src="images/1674242444207.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  montre à quartz avec jour
                </h6>
               <!--  <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>
  </section>
      <!-- <div class="btn-box">
        <a href="produt.html">
          VOIR TOUS LES PRODUITS
        </a> -->
        <section class="shop_section layout_padding" id="numérique">
    <div class="container">
      <div class="heading_container heading_center">
        <H3>
         MONTRES <b> numérique</b>
        </H3>
      </div>
      <div class="row">
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1674242594690.png">
              <div class="img-box">
                <img src="images\1674242575738.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                 Cadran numérique et mécanique
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466490071.jpg">
              <div class="img-box">
                <img src="images/1681039432124.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                Cadran numérique et mécanique
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1681040296544.png">
              <div class="img-box">
                <img src="images/1681040201684.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Cadran numérique et mécanique
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1681038889251.png">
              <div class="img-box">
                <img src="images/1681035750591.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Cadran numérique et mécanique
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1681040471695.png">
              <div class="img-box">
                <img src="images/1681040465741.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Numérique (touch watch)
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>

        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466489769.jpg">
              <div class="img-box">
                <img src="images/1695466489710.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre numérique
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466489866.jpg">
              <div class="img-box">
                <img src="images/1695466489854.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre connectable(Dame)
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466489583.jpg">
              <div class="img-box">
                <img src="images/1695466489591.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre connectable (Sportif)
                </h6>
                <!-- <h6>
                  prix
                       <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>
  </section>
   <section class="shop_section layout_padding" id="automatique">
    <div class="container">
      <div class="heading_container heading_center">
        <H3>
         MONTRES <b> automatique</b>
        </H3>
      </div>
      <div class="row">
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466489830.jpg">
              <div class="img-box">
                <img src="images/1695466489842.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre automatique
                </h6>
               <!--  <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466489975.jpg">
              <div class="img-box">
                <img src="images/1695466489937.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre automatique
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466489999.jpg">
              <div class="img-box">
                <img src="images/1695466489986.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre automatique
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466490105.jpg">
              <div class="img-box">
                <img src="images/1695466490035.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre automatique
                </h6>
             <!--    <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466489914.jpg">
              <div class="img-box">
                <img src="images/1695466490059.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre automatique
                </h6>
              <!--   <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466490023.jpg">
              <div class="img-box">
                <img src="images/1695466490083.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre automatique
                </h6>
                <!--<h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1681039167493.png">
              <div class="img-box">
                <img src="images/1681039167493.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre automatique (Dame)
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1681039175601.png">
              <div class="img-box">
                <img src="images/1681039175601.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Montre automatique (Dame)
                </h6>
              <!--   <h6>
                  prix
                       <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>
  </section>
      <!-- <div class="btn-box">
        <a href="produt.html">
          VOIR TOUS LES PRODUITS
        </a> -->
  <section class="shop_section layout_padding" id="CUIR">
    <div class="container">
      <div class="heading_container heading_center">
        <H3>
         SAC en <b>CUIR</b><p>(tous disponible en noir et maron)</p>
        </H3>
      </div>
      <div class="row">
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images/1670234377834.png">
              <div class="img-box">
                <img src="images/1670234377834.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac homme avec pochette
                </h6>
            <!--     <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images/1673256940935.png">
              <div class="img-box">
                <img src="images/1673256940935.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac homme
                </h6>
               <!--  <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1679987193324.png">
              <div class="img-box">
                <img src="images/1679987179259.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac homme
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1678297554487.png">
              <div class="img-box">
                <img src="images/1665007073226.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac homme avec pochette
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1675148436524.png">
              <div class="img-box">
                <img src="images/1675148398382.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac homme 
                </h6>
                <!-- <h6>
                  prix
                  <span>
                  00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1675180743609.png">
              <div class="img-box">
                <img src="images/1675180738185.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac homme vérouille à code
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466489684.jpg">
              <div class="img-box">
                <img src="images/1695466489672.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac homme
                </h6>
                <!-- <h6>
                  prix
                  <span>
                   00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1678297554487.png">
              <div class="img-box">
                <img src="images/1678297615468.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac homme
                </h6>
              <!--   <h6>
                  prix
                    <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>
  </section>
      <!-- <div class="btn-box">
        <a href="produt.html">
          VOIR TOUS LES PRODUITS
        </a> -->

  <section class="shop_section layout_padding" id="ORDI">
    <div class="container">
      <div class="heading_container heading_center">
        <H3>
        SAC <b>ORDI</b> & Sac en <b>bandoulière</b><p>(Sac Ordi dispo en gris noir et bleu, les autres en noir et maron)</p>
        </H3>
      </div>
      <div class="row">
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1680341581144.png">
              <div class="img-box">
                <img src="images/1671565154273.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac Ordi 15,3 pocues
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6>
 -->              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1678714235564.png">
              <div class="img-box">
                <img src="images/1678714246115.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac Ordi avec poche extérieur
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images/1676021767363.png">
              <div class="img-box">
                <img src="images/1676021767363.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac en bandoulière 
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    000
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1675343142865.png">
              <div class="img-box">
                <img src="images/1675343140021.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Saccoche à pore de charge
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images/1678872093799.png">
              <div class="img-box">
                <img src="images/1678872089656.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                 Sac SaGa avec anse
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466489901.jpg">
              <div class="img-box">
                <img src="images/1695466489889.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac SaGa
                </h6>
               <!--  <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="">
              <div class="img-box">
                <img src="images/1696248978233.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Porte-monnaie
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1696248978120.jpg">
              <div class="img-box">
                <img src="images/1696248978109.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Banane
                </h6>
                <!-- <h6>
                  prix
                   <span>
                    00
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>
  </section>
      <!-- <div class="btn-box">
        <a href="produt.html">
          VOIR TOUS LES PRODUITS
        </a> -->
   <section class="shop_section layout_padding" id="CUIR">
    <div class="container">
      <div class="heading_container heading_center">
        <H3>
         SAC à main <b>Dame</b> & Maquillage
        </H3>
      </div>
      <div class="row">
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466489603.jpg">
              <div class="img-box">
                <img src="images/1695466489603.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac à main 3 pièces 
                </h6>
            <!--     <h6>
                  prix
                  <span>
                    $200
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466489649.jpg">
              <div class="img-box">
                <img src="images/1695466489649.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac à main 3 pièces
                </h6>
               <!--  <h6>
                  prix
                  <span>
                    $300
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466489614.jpg">
              <div class="img-box">
                <img src="images/1695466489614.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac à main 3 pièces
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    $110
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466489626.jpg">
              <div class="img-box">
                <img src="images/1695466489626.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac à main 5 pièces
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    $45
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466489637.jpg">
              <div class="img-box">
                <img src="images/1695466489637.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac à main 6 pièces
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    $95
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1696248978085.jpg">
              <div class="img-box">
                <img src="images\1696248978179.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Sac à main 4 pièces
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    $70
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
         <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1681303976101.png">
              <div class="img-box">
                <img src="images/1681303972616.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Maquillage (moyen)
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    $400
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1675437106023.png">
              <div class="img-box">
                <img src="images/16754371025211.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Maquillage (Grand)
                </h6>
              <!--   <h6>
                  prix
                    <span>
                    $450
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section class="shop_section layout_padding" id="Autres">
    <div class="container">
      <div class="heading_container heading_center">
        <H3>
         Autres de nos produits
        </H3>
      </div>
      <div class="row">
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466490118.jpg">
              <div class="img-box">
                <img src="images/1695466490118.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Bracelet
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    $200
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466490360.jpg">
              <div class="img-box">
                <img src="images/1695466490347.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Bracelet
                </h6>
                <!-- <h6>
                  prix
                  <span>
                    $300
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466490383.jpg">
              <div class="img-box">
                <img src="images/1695466490383.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Bracelet
                </h6>
               <!--  <h6>
                  prix
                  <span>
                    $110
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="">
              <div class="img-box">
                <img src="images/1695466490314.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Bracelet
                </h6>
                <!-- <h6>
                 
                  <span>
                  PRIX
                  </span>
                </h6> -->
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466490336.jpg">
              <div class="img-box">
                <img src="images/1696248978192.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Bracelet
                </h6>
              
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1696248978363.jpg">
              <div class="img-box">
                <img src="images/1696248978352.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Masque de soin
                </h6>
              
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1683217789883.png">
              <div class="img-box">
                <img src="images/1683217786788.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Culottes en coton
                </h6>
              
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466490233.jpg">
              <div class="img-box">
                <img src="images/1695466490221.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  culottes en polystyrène
                </h6>
              
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
            <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1681735420738.png">
              <div class="img-box">
                <img src="images/1681735416468.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Mini boîte de macquillage
                </h6>
                
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1695466490268.jpg">
              <div class="img-box">
                <img src="images/1695466490291.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                 Pinceaux de macquillage
                </h6>
              
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1677502823018.png">
              <div class="img-box">
                <img src="images/1677502829909.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Boxer
                </h6>
               
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3">
          <div class="box">
            <a href="images\1682006448527-Récupéré.png">
              <div class="img-box">
                <img src="images/1682006451056.png" alt="">
              </div>
              <div class="detail-box">
                <h6>
                  Chaine de pied
                </h6>
               
              </div>
              <div class="new">
                <span>
                  New
                </span>
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>
  </section>


  <!-- end shop section -->

  <!-- info section -->
<section class="info_section  layout_padding2-top">
    <div class="social_container">
      <div class="social_box">
        <a href="https://www.facebook.com/modetendanceny?mibextid=ZbWKwL" target="_blank">
          <i class="fa fa-facebook" aria-hidden="true"></i>
        </a>
        <a href="https://wa.me/+22789735585" target="_blank">
          <i class="fa fa-whatsapp" aria-hidden="true"></i>
        </a>
        <a href="https://www.instagram.com/modetendanceny" target="_blank">
          <i class="fa fa-instagram" aria-hidden="true"></i>
        </a>

      </div>
    </div>
    <div class="info_container ">
      <div class="container">
        <div class="row">
          <div class="col-md-6 col-lg-3">
            <h6>
              A PROPOS
            </h6>
            <p>
              Chez MODE ET TENDANCE,Nous vous proposons une large gamme de sacs,montres ,et bien plus encore,conçus pour ajouter une touches d’élégance  à votre look. 

            </p>
          </div>
          <div class="col-md-6 col-lg-3">
            <div class="info_form ">
              <h5>
                <strong>MESSAGES</strong>
              </h5>
              <p>Envoyez un e-mail à la boutique
                pour plus d'informations ou passez<br>
                une commande
              </p>
              <form action="mailto:modetendanceny">
                <button>
                  Ecrire
                </button>
              </form>
            </div>
          </div>
          <div class="col-md-6 col-lg-3">
            <h6>
              BESOIN D'AIDE ?
            </h6>
            <p>
              Vous cherchez un produit en quantité et qui n'est pas disponible.Vous pouvez passer une commande direct.Il vous sera remis dès son arrivée
            </p>
          </div>
          <div class="col-md-6 col-lg-3">
            <h6>
              CONTACTEZ NOUS
            </h6>
            <div class="info_link-box">
              <a>
                <i class="fa fa-map-marker" aria-hidden="true"></i>
                <span>Qtr Ryad Plaque Avocat </span>
              </a>
              <a>
                <i class="fa fa-phone" aria-hidden="true"></i>
                <span>+227 89 73 55 85</span>
              </a>
              <a href="mailto:modetendanceny@gmail.com">
                <i class="fa fa-envelope" aria-hidden="true"></i>
                <span>modetendanceny@gmail.com</span>
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>
</section>
  <!-- footer section -->

<!-- end info section -->


<script src="js/jquery-3.4.1.min.js"></script>
<script src="js/bootstrap.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/owl.carousel.min.js">
</script>
<script src="js/custom.js"></script>

<?php
require_once 'footer.php';
?>